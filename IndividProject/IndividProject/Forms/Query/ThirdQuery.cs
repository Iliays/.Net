...
namespace IndividProject.Forms
{
	public partial class ThirdQuery : Form
	{
		...
		private void buttonUpload_Click(object sender, EventArgs e)
		{
			...
			DateTime give = dateTimePickerDateGive.Value;
			DateTime take = dateTimePickerDateTake.Value;
			...
			using (var context = new LombardContext())
			{
				var queryForth = from order in context.Orders
								 ...
								 where (order.DateGive >= give && order.DateGive <= take) && (order.DateTake >= give && order.DateTake <= take) && order.ClientId == client.ClientId && order.CategoryId == category.CategoryId
								 select new
								 {
